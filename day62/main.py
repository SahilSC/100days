...
class CafeForm(FlaskForm):
    ...
    open=StringField('Opening Time (Ex. 8:00AM', validators=[DataRequired()])
    close=StringField('Closing Time (Ex. 5:00pm', validators=[DataRequired()])
...
def add_cafe():
    ...
    if form.validate_on_submit():
        with open('cafe-data.csv', 'a', newline='') as cafedata:
            writer = csv.writer(cafedata)
